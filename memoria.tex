...
\tutor{Xavier Burgués Illa\\Enginyeria de Serveis i Sistemes d'Informació}
\curs{28-06-2017}
...
\noindent La tecnología ha ido evolucionando des de hace unos años. Dados los hechos, todavía hay sectores en los que no se les muestra suficiente interés. Uno de estos sectores es el de las aplicaciones para personas con diversidad funcional de déficit cognitivo.\\ \\
Aquí el objetivo de este trabajo de fin de grado, crear un \textit{software} que no los haga sentir un público distinto, una red social adaptada para personas que presenten este problema.\\ \\
El trabajo consistirá en desarrollar una red social adaptada para personas con diversidad funcional de déficit cognitivo, con la adición de una herramienta para los profesionales en el ámbito, con la que podrán gestionar eventos de distintas organizaciones.
\end{abstract}
\begin{abstract}[english]
\noindent Technology has evolved from a few years ago. Despite this, there are still sectors where they are not shown sufficient interest. One of these sectors is the one of the applications for people with functional diversity of cognitive deficit. \\ \\
Here is the purpose of this end-of-grade paper, to create a \textit{software} that does not make them feel different, a social network adapted for people who present this problem. \\ \\
The work will consist of developing a social network adapted for people with functional diversity of cognitive deficit, with the addition of a tool for professionals in the field, with which they can manage events of different organizations.
\end{abstract}
...
\noindent A continuació trobem una taula amb el cost de mercat i el cost estimat d’aquest projecte:
...
Addicionalment, l'última versió de l'aplicació es troba desplegada a la següent URL:\\
\url{https://makeyourfriend.herokuapp.com}\\ \\
Per poder crear un usuari en aquesta pàgina, s'haurà d'introduir el següent codi de validació:\\ \textbf{3818207-5651-416d-ac34-e49228d6ee7f}
\section{Disseny}
...
\end{lstlisting}
Com podem veure, en primer lloc, es valida que el formulari s'hagi omplert correctament. Seguidament s'executa una  \textit{query} per agafar tots els usuaris del sistema. Aquesta \textit{query}, anirà al fitxer app.js (capa de negoci) i seguidament al fitxer neo4j-api.js (capa de dades), Agafarà els usuaris del sistema i els mostrarà al client. A partir d'aquí, es validarà que el codi de validació introduït per l'usuari sigui vàlid i que l'usuari no existeixi al sistema.
...
\end{lstlisting}
\end{itemize}
\item Una vegada que s'ha agafat la informació necessària de la capa de dades, es renderitza la pàgina web que el client vol veure passant-li tota aqueta informació.
\end{enumerate}
